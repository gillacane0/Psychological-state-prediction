...
df.replace({'Cognitive Load': 'Low'}, 0, inplace=True)
df.replace({'Cognitive Load': 'Moderate'}, 1, inplace=True)
df.replace({'Cognitive Load': 'High'}, 2, inplace=True)
pd.set_option('future.no_silent_downcasting', True)

t = df['Cognitive Load'].values
X = df.drop(['Cognitive Load'],axis=1)
...
# VIUSALIZE BEST PARAMETERS OF THE ALGORIHTMS
